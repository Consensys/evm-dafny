	...
	decreases EGas(st)
{
	...
	else if !(st.Operands() >= 0) then 
		assert false;
		INVALID(STACK_UNDERFLOW)
	else if !(st.Capacity() >= 6) then 
		assert false;
...
{
    if st.Gas() < 1 then INVALID(INSUFFICIENT_GAS)
	else if !(st.Operands() > 1) then 
		assert false;
		INVALID(STACK_UNDERFLOW)
	else if !(st.Capacity() > 2) then 
		assert false;
		INVALID(STACK_OVERFLOW)
    else 
        var st0 := JumpDest(st);
...
{
	if st.Gas() < 1 then INVALID(INSUFFICIENT_GAS)
	else if !(st.Operands() >= 3) then 
		assert false;
		INVALID(STACK_UNDERFLOW)
	else if !(st.Capacity() > 1) then 
		assert false;
		INVALID(STACK_OVERFLOW) 
	else
	    var st0 := JumpDest(st);
